...
function prev() {
	inputBox.value = "";
	const $divs = document
		.querySelectorAll('#input-list > div');
	$divs.forEach(function(e) {
		e.remove();
	});
	const $btns = document
		.querySelectorAll('#input-list > button');
	$btns.forEach(function(e1) {
		e1.remove();
	});
	if (pageFirst.getMonth() === 1) {
		pageFirst = new Date(first
			.getFullYear() - 1, 12, 1);
		first = pageFirst;
		if (first.getFullYear() % 4 === 0) {
			pageYear = leapYear;
	...
	currentTitle.innerHTML = monthList[first
		.getMonth()]
		+ '&nbsp;&nbsp;&nbsp;&nbsp;'
		+ first.getFullYear();
	removeCalendar();
	showCalendar();
	showMain();
	clickedDate1 = document
		.getElementById(today.getDate());
...
//todo list 입력 , 삭제
//기존 리스트값을 초기화하고 현제 todolist에 있는 값을 다시 리스트업한다.
//기존 리스트 초기화
...
var dataCnt = 1;
var keyValue = today.getFullYear() + ''
	+ today.getMonth() + ''
	+ today.getDate();
let todoList = [];
todoList[keyValue] = [];
...
function addTodoList() {
	...
	inputList.appendChild($div);
	inputList.appendChild($btn);
	//inputbox에서 입력한 값 선택된 날짜 배열에 넣기
